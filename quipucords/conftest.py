...
def scan_manager(session_mocker):
    """Return a ScanManager instance for testing."""
...
def test_uri_map():
    ...
    from tests.constants import BaseURI, ConstantsFromEnv, EnvVar
...
@pytest.fixture(scope="module")
def vcr_config(test_uri_map):
    """
    VCR configuration - should match the kwargs expected by VCR.use_cassetes.

    https://github.com/kiwicom/pytest-recording#configuration
    https://vcrpy.readthedocs.io/en/latest/configuration.html
    """
    replace_cassete_uri = partial(_replace_cassete_uri, uri_map=test_uri_map)
    return {
        "filter_headers": [
