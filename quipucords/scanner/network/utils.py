...
def construct_inventory(
    ...
    *,
    ...
    :param ssh_args: a list of extra ssh arguments
    ...
    for index, group in enumerate(concurreny_groups):
        ...
        children[group_name] = {
            "hosts": _format_hosts_dict(group, ssh_executable, ssh_args)
        }

    return group_names, inventory


def _format_hosts_dict(group, ssh_executable, ssh_args) -> dict:
    hosts_dict = {}
    for host in group:
        host_vars = _get_host_vars(host)
        if ssh_executable:
